...
public class Solitaire {
	...
	void jokerA() {
		...
		CardNode toBeSwapped = ptr.next;
		CardNode post = toBeSwapped.next;
		...
		toBeSwapped.next = ptr;
		ptr.next = post;


		if(toBeSwapped.cardValue == deckRear.cardValue){
			//if the jokerA is second to last, then when it is switched, it must become the new deckRear
			deckRear = ptr;
		}

		if (ptr.cardValue == deckRear.cardValue){
			deckRear = toBeSwapped;
		}
	...
	void jokerB() {
		...
		else if(next.cardValue == deckRear.cardValue){
			deckRear = last;
		}
